...
{
	/*  功率控制B,Power control B (CFh)  */
	DEBUG_DELAY();
	LCD_ILI9341_CMD(0xCF);
	LCD_ILI9341_Parameter(0x00);
	LCD_ILI9341_Parameter(0x81);
	LCD_ILI9341_Parameter(0x30);
	/*  上电顺序控制Power on sequence control (EDh) */
	DEBUG_DELAY();
	...
	LCD_ILI9341_Parameter(0x81);
	/*  司机定时控制A,Driver timing control A (E8h) */
	DEBUG_DELAY();
	LCD_ILI9341_CMD(0xE8);
	LCD_ILI9341_Parameter(0x85);
	LCD_ILI9341_Parameter(0x10);
	LCD_ILI9341_Parameter(0x78);	
	/*  功率控制A,Power control A (CBh) */
	DEBUG_DELAY();
	...
	/* 泵比控制Pump ratio control (F7h) */
	DEBUG_DELAY();
	LCD_ILI9341_CMD(0xF7);
	LCD_ILI9341_Parameter(0x20);	
	/* 驱动定时控制B,Driver timing control B */
	DEBUG_DELAY();
	LCD_ILI9341_CMD(0xEA);
	LCD_ILI9341_Parameter(0x00);
	LCD_ILI9341_Parameter(0x00);	
	/* 帧速率控制器Frame Rate Control (In Normal Mode/Full Colors) (B1h) */
	DEBUG_DELAY();
	LCD_ILI9341_CMD(0xB1);
	LCD_ILI9341_Parameter(0x00);
	LCD_ILI9341_Parameter(0x1B);	
	/*  显示功能控制Display Function Control (B6h) */
	DEBUG_DELAY();
	LCD_ILI9341_CMD(0xB6);
	LCD_ILI9341_Parameter(0x0A);
	LCD_ILI9341_Parameter(0xC2);//	0xA2
	/* 功率控制1,Power Control 1 (C0h) */
	DEBUG_DELAY();
	LCD_ILI9341_CMD(0xC0);
	LCD_ILI9341_Parameter(0x35);	
	/* 功率控制2,Power Control 2 (C1h) */
	DEBUG_DELAY();
	...
	DEBUG_DELAY();	
	/* 积极的伽马校正Positive Gamma Correction */
	...
	LCD_ILI9341_Parameter(0x00);	
	/* 消极的伽马校正Negative Gamma Correction (E1h) */
	...
	LCD_ILI9341_Parameter(0x0F);	
/*---------------------------修改这部分代码即可修改显示的方向---------------------------*/	
	/* 内存访问控制设置memory access control set */
	DEBUG_DELAY();
	LCD_ILI9341_CMD(0x36); 	
	LCD_ILI9341_Parameter(0x68);    /*竖屏  左上角到(起点)到右下角(终点)扫描方式*/
	DEBUG_DELAY();	
	/* 列地址控制设置column address control set */
	LCD_ILI9341_CMD(0X2A); 
	LCD_ILI9341_Parameter(0x00);
	LCD_ILI9341_Parameter(0x00);
	LCD_ILI9341_Parameter(0x01);
	LCD_ILI9341_Parameter(0x3F);	
	/* 页面地址控制设置page address control set */
	DEBUG_DELAY();
